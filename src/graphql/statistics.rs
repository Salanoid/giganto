use super::{base64_engine, get_filtered_iter, get_timestamp, network::key_prefix, Engine};
use crate::graphql::ConverStr;
use crate::{
    graphql::{RawEventFilter, TimeRange},
    ingest::{AnalyzerStatistics, CollectorStatistics},
...
fn collector_stats_connection(
    ...
    loop {
        if let Some((key, value)) = statistics_data {
            ...
            result_vec.push(Edge::new(base64_engine.encode(&key), collector_stats));
    ...
    loop {
        if let Some((key, value)) = statistics_data {
            ...
            if !analyzer_stats.stats.is_empty() {
                result_vec.push(Edge::new(base64_engine.encode(&key), analyzer_stats));
            }
            statistics_data = stats_iter.next();
        }
...
mod tests {
    ...
    use crate::ingest::{AnalyzerStatistics, CollectorStatistics, RealTimeStatistics};
    use crate::storage::RawEventStore;
    use chrono::Utc;
    use giganto_client::ingest::RecordType;
